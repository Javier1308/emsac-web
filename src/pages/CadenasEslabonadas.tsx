          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 2 }}>
            {specs.map((s, i) => (
              <div key={i} style={{
                ...
                <p style={{ fontFamily: 'var(--font-display)', fontSize: 10, letterSpacing: '3px', textTransform: 'uppercase', color: '#E85D04', marginBottom: 8 }}>{s.label}</p>
                <p style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 15, color: '#fff', lineHeight: 1.4 }}>{s.value}</p>
              </div>
            ))}
          </div>
        </div>
